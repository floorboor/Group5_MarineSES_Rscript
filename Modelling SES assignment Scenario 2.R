#SCENARIO 2 Targeting the fins of the Scalloped Hammerhead Shark
...
parms_L <- c(r = 0.5, K = 60, q = 0.007, p = 1.5, c = 1, theta = 0.02)
...
ggplot(df_null, aes(x = S, y = E)) +
  ...
  labs(title = "Nullclines in the (S, E) plane (Logistic) Scenario 2",
...
ggplot(traj_L_long, aes(x = time, y = val, color = ic)) +
  ...
  theme_minimal() +
  labs(title = "ODE-system Scenario 2", y = "Size", x = "Time", color = "Initial condition")
...
parms_A <- c(r = 0.5, K = 60, A = 20, q = 0.007, p = 1.5, c = 1, theta = 0.1)
...
ggplot(df_null, aes(x = S, y = E)) +
  ...
  labs(
    title = "Interior nullclines in the (S, E) plane (Allee effect) Scenario 2",
...
ggplot(traj_A_long, aes(x = time, y = val, color = ic)) +
  ...
  facet_wrap(~var, scales = "free_y", ncol = 1) +
  theme_minimal() +
  labs(title = "Open access trajectories (Allee) Scenario 2", y = "Size", x = "Time",color= "Initial condition")
...
ggplot(df_long, aes(x = time, y = val, color = var)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Scenario 2 Social–ecological system: cooperators and fish stock",
       x = "Time", y = NULL)

...
ggplot(df_null, aes(x = S, y = E)) +
  ...
  labs(title = "Nullclines in the (S, E) plane (Logistic) Scenario 3",
...
ggplot(traj_L_long, aes(x = time, y = val, color = ic)) +
  ...
  theme_minimal() +
  labs(title = "ODE-system Scenario 3", y = "Size", x = "Time", color = "Initial condition")
...
ggplot(df_null, aes(x = S, y = E)) +
  ...
  labs(
    title = "Interior nullclines in the (S, E) plane (Allee effect) Scenario 3",
...
ggplot(traj_A_long, aes(x = time, y = val, color = ic)) +
  ...
  facet_wrap(~var, scales = "free_y", ncol = 1) +
  theme_minimal() +
  labs(title = "Open access trajectories (Allee) Scenario 3", y = "Size", x = "Time",color= "Initial condition")
...
ggplot(df_long, aes(x = time, y = val, color = var)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Scenario 3 Social–ecological system: cooperators and fish stock",
       x = "Time", y = NULL)

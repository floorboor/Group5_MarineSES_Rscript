#SCENARIO 5 High Seas Treaty Implemented
...
ggplot(df_null, aes(x = S, y = E)) +
  ...
  labs(title = "Nullclines in the (S, E) plane (Logistic) Scenario 5",
...
ggplot(traj_L_long, aes(x = time, y = val, color = ic)) +
  ...
  theme_minimal() +
  labs(title = "ODE-system Scenario 5", y = "Size", x = "Time", color = "Initial condition")
...
ggplot(df_null, aes(x = S, y = E)) +
  ...
  labs(
    title = "Interior nullclines in the (S, E) plane (Allee effect) Scenario 5",
...
ggplot(traj_A_long, aes(x = time, y = val, color = ic)) +
  ...
  facet_wrap(~var, scales = "free_y", ncol = 1) +
  theme_minimal() +
  labs(title = "Open access trajectories (Allee) Scenario 5", y = "Size", x = "Time",color= "Initial condition")
...
ggplot(df_long, aes(x = time, y = val, color = var)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Scenario 5 Social–ecological system: cooperators and fish stock",
       x = "Time", y = NULL)
